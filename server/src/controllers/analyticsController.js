...
import { EXPORT_SIGNING_SECRET } from '../configs/analytics.js'
...
export const getTopViewed = asyncHandler(async (req, res) => {
  ...
  const blogFilter = buildBlogFilter(filter)
  if (req.query.search) {
    blogFilter.title = { $regex: req.query.search, $options: 'i' }
  }
  const blogs = await Blog.find(blogFilter).select('_id').lean()
  const blogIds = blogs.map((b) => b._id)
...
export const exportCsv = asyncHandler(async (req, res) => {
  ...
  for (const b of blogs) {
    const views = await BlogView.countDocuments({
      blog: b._id,
      isAdminView: false,
      viewedAt: { $gte: filter.from, $lte: filter.to }
    })
    const commentCount = await Comment.countDocuments({ blog: b._id })
  ...
  const signature = crypto
    .createHmac('sha256', EXPORT_SIGNING_SECRET)
    .update(csv)
    .digest('hex')
...
export const recordView = asyncHandler(async (req, res) => {
  ...
  if (authHeader) {
    const token = authHeader.startsWith('Bearer ')
      ...
      : authHeader
    try {
      console.log('[analytics] auth header:', req.headers.authorization)
